...
class Graph:
    def __init__(self, max_points, t1, teams, matches, points_table):
        ...
        for match in matches:
            self.node_indexes[f"match-{match['teamA']}-{match['teamB']}-{match['id']}"] = index
            index += 1

        for team in teams:
            self.node_indexes[f"team-{team['name']}"] = index
            index += 1
...
def predict_team_success(team_name):
    
    points_tabel_json = load_data('points_table.json')
    ...
    schedule_json = load_data('schedule.json')
    matches = [
